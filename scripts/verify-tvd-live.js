...
const engine=context.window.BT001TakerVolumeDeltaCore.createEngine({durationMs:5000,lookback:2});
...
function finish(error){
  ...
  console.log(JSON.stringify({
    ...
    forming:{trades:snapshot.current.tradeCount,buyVolume:snapshot.buyVolume,sellVolume:snapshot.sellVolume,totalVolume:snapshot.totalVolume,buyPct:snapshot.buyPct,sellPct:snapshot.sellPct,delta:snapshot.delta,totalLengthPct:snapshot.totalLengthPct}
  },null,2));
}
...
socket.on("message",payload=>{
  ...
  engine.ingest({symbol:trade.s,exchangeTime:trade.T||trade.E,quantity:trade.q,buyerIsMaker:trade.m});
