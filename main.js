...
function closedTradeIncomeValue(row){
  return closedTradeNumber(row && row.income);
}
...
function closedTradeFastSummarySideFromRaw(rawSide){
  return String(rawSide || "").toUpperCase() === "BUY" ? "SHORT" : "LONG";
}

function closedTradeFastSummaryDirection(side){
  return String(side || "").toUpperCase() === "SHORT" ? "S" : "L";
}

function closedTradeFastWeightedPrice(rows){
  const items = Array.isArray(rows) ? rows : [];
  const totalQty = items.reduce((sum,row) => sum + closedTradeNumber(row && row.qty),0);
  if(!(totalQty > 0)) return null;
  const totalValue = items.reduce((sum,row) => sum + closedTradeNumber(row && row.qty) * closedTradeNumber(row && row.price),0);
  return totalValue / totalQty;
}

function buildClosedTradeFastResolvedSummaries(rec,win){
  const periodRec = filterClosedReconstructionForPeriod(rec,win);
  const parents = closedTradeParentTrades(periodRec);
  return parents.map((trade,index) => {
    const entries = Array.isArray(trade && trade.entries) ? trade.entries.slice() : [];
    const exits = Array.isArray(trade && trade.exits) ? trade.exits.slice() : [];
    const links = Array.isArray(trade && trade.links) ? trade.links.slice() : [];
    const qty = links.length
      ? links.reduce((sum,link) => sum + closedTradeNumber(link && link.qty),0)
      : exits.reduce((sum,marker) => sum + closedTradeNumber(marker && marker.qty),0);
    const realized = links.length
      ? links.reduce((sum,link) => sum + closedTradeRealizedValue(link),0)
      : exits.reduce((sum,marker) => sum + closedTradeNumber(marker && (marker.binanceRealizedPnl ?? marker.realizedPnl ?? marker.pnl)),0);
    const fees = links.length
    ...
    return {
      id:"wf_fast_pos_" + index + "_" + String(trade && trade.parentId || ""),
      parentTradeId:trade && trade.parentId ? trade.parentId : null,
      detailChainId:trade && trade.parentId ? trade.parentId : null,
      markerId:closeMarker && closeMarker.id ? closeMarker.id : null,
      symbol:String((entries[0] && entries[0].symbol) || (closeMarker && closeMarker.symbol) || cfg().symbol || "").toUpperCase(),
      side,
      dir:closedTradeFastSummaryDirection(side),
      openTime:closedTradeNumber(entries[0] && entries[0].time) || null,
      closeTime:closedTradeNumber(closeMarker && closeMarker.time) || null,
      qty,
      realized,
      fees,
      net:realized + fees,
      ...
      limited:false,
      limitedReason:"",
      tradeIds:[...new Set(links.map(link => link && link.tradeId).filter(value => value != null))],
      orderIds:[...new Set(links.map(link => link && link.orderId).filter(value => value != null))]
    };
  });
}

function buildClosedTradeFastLimitedSummaries(rec,win){
  const startSec = Math.floor(win.start / 1000);
  const endSec = Math.floor(win.end / 1000);
  const unresolved = (rec && rec.markers || [])
    .filter(marker =>
      marker &&
      marker.role === "close" &&
      marker.unresolved &&
      Number(marker.time) >= startSec &&
      Number(marker.time) <= endSec
    )
    .sort((a,b) => closedTradeNumber(a && a.time) - closedTradeNumber(b && b.time));
  const groups = [];
  let current = null;
  unresolved.forEach(marker => {
    const side = closedTradeFastSummarySideFromRaw(marker && marker.rawSide);
    const time = closedTradeNumber(marker && marker.time);
    const orderId = marker && marker.orderId != null ? marker.orderId : null;
    const shouldSplit = !current ||
      current.side !== side ||
      (orderId != null && current.orderId != null && String(orderId) !== String(current.orderId)) ||
      (time - current.lastTime) > 1800;
    if(shouldSplit){
      current = {
        side,
        orderId,
  ...
  return groups.map((group,index) => {
    ...
    const qty = markers.reduce((sum,marker) => sum + closedTradeNumber(marker && marker.qty),0);
    const realized = markers.reduce((sum,marker) => sum + closedTradeNumber(marker && (marker.binanceRealizedPnl ?? marker.realizedPnl ?? marker.pnl)),0);
    const fees = markers.reduce((sum,marker) => sum + closedTradeSignedFeeValue(marker && marker.fee),0);
    const lastMarker = markers[markers.length - 1] || null;
    return {
      id:"wf_fast_limited_" + index + "_" + String(group.side || "") + "_" + String(lastMarker && lastMarker.time || 0),
      parentTradeId:null,
      detailChainId:null,
      markerId:lastMarker && lastMarker.id ? lastMarker.id : null,
      symbol:String(lastMarker && lastMarker.symbol || cfg().symbol || "").toUpperCase(),
      side:group.side,
      dir:closedTradeFastSummaryDirection(group.side),
      openTime:null,
      closeTime:closedTradeNumber(lastMarker && lastMarker.time) || null,
      qty,
      realized,
      fees,
      net:realized + fees,
      ...
      limited:true,
      limitedReason:"Partial context",
      tradeIds:[...new Set(markers.map(marker => marker && marker.tradeId).filter(value => value != null))],
      orderIds:[...new Set(markers.map(marker => marker && marker.orderId).filter(value => value != null))]
    };
  });
}

function buildClosedTradeFastReport(rec,win,symbol,contextLimited){
  const resolved = buildClosedTradeFastResolvedSummaries(rec,win);
  const limited = buildClosedTradeFastLimitedSummaries(rec,win);
  const summaries = resolved.concat(limited).sort((a,b) =>
    closedTradeNumber(a && a.closeTime) - closedTradeNumber(b && b.closeTime) ||
    String(a && a.id || "").localeCompare(String(b && b.id || ""))
  );
  const wins = summaries.filter(item => closedTradeNumber(item && item.net) > 0);
  const losses = summaries.filter(item => closedTradeNumber(item && item.net) < 0);
  const profit = wins.reduce((sum,item) => sum + closedTradeNumber(item && item.net),0);
  const loss = losses.reduce((sum,item) => sum + closedTradeNumber(item && item.net),0);
  return {
    mode:"fast",
    symbol:String(symbol || "").toUpperCase(),
    period:win,
    summaries,
    contextLimited:!!contextLimited || summaries.some(item => item && item.limited),
    summary:{
      wins:wins.length,
...
const marketDataHub = (() => {
  ...
  function getAuthoritativeMaSnapshot(tf,options={}){
    ...
    const maxRows = Number.isFinite(Number(options.maxRows))
      ? Math.max(requiredRows,Math.round(Number(options.maxRows)))
      : 0;
    const includeForming = options.includeForming !== false;
    const revisions = getTimeframeRevisions(interval);
    const cacheKey = [revisions.symbol,interval,includeForming ? "live" : "closed",periods.join("-"),requiredRows,maxRows || "all",revisions.closedRevision,includeForming ? revisions.formingRevision : "-"].join("|");
...
function pnlLabel(txt,x,y,col,clip,opt=null){
  ...
  const anchorGap = align === "left-of-point" ? Math.max(4, Number(opt && opt.gap) || 0) : Math.max(8, Number(opt && opt.gap) || 0);
...
(() => {
  ...
  function reserveLabel15(txt,x,y,col,clip,placed,opt={}){
    ...
    const w = ctx.measureText(txt).width + pad*2;
    const h = opt.h || 16;
    const align = opt.align === 'left-of-point' ? 'left-of-point' : 'center';
    const gap = align === 'left-of-point' ? Math.max(4, Number(opt.gap) || 0) : Math.max(8, Number(opt.gap) || 0);
    const fixedX = !!opt.fixedX;
    const fillBg = opt.fillBg !== false;
    const edge = opt.edge || null;
    const baseXRaw = align === 'left-of-point'
      ? x - gap - w/2
      : x;
    const baseX = baseXRaw + (Number(opt.xShift) || 0);
    const baseY = edge === 'top' ? (clip.top + (Number(opt.edgeMargin) || 26)) : edge === 'bottom' ? (clip.top + clip.height - (Number(opt.edgeMargin) || 26)) : y;
    const offsets = opt.offsets || (
      align === 'left-of-point'
        ? [0]
        : (edge === 'top' ? [0,24,48,72,96,120] : edge === 'bottom' ? [0,-24,-48,-72,-96,-120] : [-12,12,-28,28,-44,44,-60,60,-76,76])
    );
    let chosen = null;
      ...
      if(m.role === 'close' && !m.unresolved && inTime(m.time,vis)){
        ...
        if(x !== null){
          ...
          const labelOpt15 = isExLabel15
            ...
            : {align:'left-of-point',gap:6,hit:{markerId:m.id,chainId:cid15(m),parentTradeId:cid15(m)}};
  ...
  function drawOpenOverlay15(vis,mapX,mapY,slot,clip,placedLabels){
    ...
    for(const m of openPositionMarkersForRender()){
      ...
      reserveLabel15((typeof fmPnlBox === 'function' ? fmPnlBox(val) : fm(val).replace(/[+-]/,'')),x,y,col,clip,placedLabels,{align:'left-of-point',gap:6,hit:{markerId:m.id,chainId:cid15(m),parentTradeId:cid15(m),owner:'open'}});
    }
...
(() => {
  ...
  /* =======================================================
     V13_01 — SETTINGS ENTRY
  ======================================================= */

  function v13InstallSettingsButton(){
    v13LabBtn = q("v13LabBtn");
    if(!v13LabBtn || v13LabBtn.__v13Bound) return;
    v13LabBtn.__v13Bound = true;
    v13LabBtn.addEventListener("click", () => {
      try{ if(typeof closeSettings === "function") closeSettings(); }catch(_e){}
      v13OpenLab();
    });
  }
  ...
  tradeOverlays = function(vis,mapX,mapY,slot,clip){
    ...
    for(const m of fillMarkers){
      ...
      if(showR && showD && m.role === 'close' && !m.unresolved){
        ...
        const isExLabel = !!(m.isFinalExit || m.letter === 'EX');
        pnlLabel(fm(val),x,isExLabel ? (y - 18) : y,lblCol,clip,isExLabel ? null : {align:'left-of-point',gap:6});
      }
    }
    ...
    for(const m of fillMarkers){
      ...
      if(showR && showD && m.role === 'close' && !m.unresolved){
        ...
        if(!drawnExitLabels.has(key)){
          ...
          const lblCol = val >= 0 ? '#1e88e5' : '#f6465d';
          const isExLabel = !!(m.isFinalExit || m.letter === 'EX');
          pnlLabel(fm(val),x,isExLabel ? (y - 18) : y,lblCol,clip,{align:isExLabel ? 'center' : 'left-of-point',gap:6});
        }
      }
    }
    ...
    if(mouse){
      ...
      if(mouse.x >= left && mouse.x <= w-right && mouse.y >= top && mouse.y <= top+priceH){
        ...
        if(floatOn && hasOpen && Number.isFinite(exp)){
          ...
          ctx.save();
          ctx.font = 'bold 11px Arial';
          const pw = Math.min(Math.max(34,canvas.clientWidth - 4),Math.max(hoverAxisBox ? hoverAxisBox.w : 34,ctx.measureText(line).width + 10));
          const pxBox = hoverAxisBox ? Math.max(2,hoverAxisBox.axisRight - pw - 2) : Math.max(2,w - pw - 2);
...
(() => {
  ...
  function drawMode2Marker15(m,vis,mapX,mapY,slot){
    ...
    let col = m.side === 'SHORT' || m.letter === 'S' || m.letter === 'ES' ? '#f6465d' : '#0ecb81';
    if(m.role === 'close') col = m.unresolved ? '#f59e0b' : (m.side === 'SHORT' ? '#f6465d' : '#0ecb81');
    const metrics = drawClosedMarker15(ix(x),ix(y),m.letter,col,m.unresolved,slot);
    pushOverlayHitItem({kind:'marker',markerId:m.id,role:m.role,side:m.side,letter:m.letter,x,y,radius:metrics.hitRadius,qty:m.qty,price:m.price,time:m.time,pnl:m.pnl,fee:m.fee || 0,unresolved:m.unresolved,chainId:cid15(m),parentTradeId:cid15(m),note:m.note || ''}, 'closed');
  }
...
(() => {
  ...
  window.MA_STACK_RUNTIME = {
    getById(id){ return $id(id); },
    ivSec(value){ return ivSec(value); },
    getConfig(){ return cfg(); },
    getInterval(){ return iv(); },
    createEvent(type,options){ return new Event(type,options); }
  };
  const MA_STACK_STRIP = window.MA_STACK_STRIP;
  if(!MA_STACK_STRIP) throw new Error("MA_STACK_STRIP module is unavailable");
...
(() => {
  ...
  function phaseClass(text){ const s=String(text||'').toLowerCase(); if(s.includes('compressed'))return 'sssc-phase-compression'; if(s.includes('trend')&&s.includes('expansion'))return 'sssc-phase-trend'; if(s.includes('transition'))return 'sssc-phase-transition'; if(s.includes('decelerating'))return 'sssc-phase-fading'; return 'sssc-phase-neutral'; }
  ...
  function eventForLevel(price,emaVal){ if(price==null||emaVal==null)return 'n/a'; const d=(price-emaVal)/price*10000; if(Math.abs(d)<8)return 'Near'; return price>emaVal?'Above':'Below'; }
  function clusterState(vals,price){ const spread=(Math.max(...vals)-Math.min(...vals))/(price||vals[0])*10000; if(spread<18)return 'Tight'; if(spread<42)return 'Moderate Separation'; return 'Wide Separation'; }
  function diagnose(label,tf,rows,snapshot,targets){
    ...
    const warmup=targets||warmupTargets(slots);
    if(rows.length<warmup.minimum){
    ...
    const directionalStrength=clamp(slopePow*0.52+sprPow*0.42+(clean<20?-18:0),-100,100);
    ...
    const phase=clean<18?'Compressed':Math.abs(directionalStrength)>35&&strengthAgrees?(direction>0?'Bullish Trend / Expansion':'Bearish Trend / Expansion'):directionalStrength<-25?'Trend Decelerating':Math.abs(direction)<25?'Directionally Mixed':'Transition';
    ...
    const vwapEvent=vw==null?'Unavailable':price>vw?'Above':'Below';
    const events={x12:c12.label,x23:c23.label,x34:c34.label,x45:c45.label,ma1:eventForLevel(price,vals[0]),ma2:eventForLevel(price,vals[1]),ma3:eventForLevel(price,vals[2]),vwap:vwapEvent,cluster:clusterState(vals,price),earlyWarning:'None'};
    return {
      tf:label,
      interval:tf,
      available:true,
      ...
      events,
      earlyWarning:null,
      reliability:rows.length>=warmup.full?'full-warmup':'minimum-warmup',
      warmupLimited:false,
      maSource:snapshot ? {
  ...
  function normalizedSsscPositionContext(){
    try{
      if(!Array.isArray(openPositionBoxes)||!openPositionBoxes.length)return {hasPosition:false,side:null};
      const side=String(openPositionBoxes[0]?.side||'').toUpperCase();
      return {hasPosition:true,side:side==='LONG'||side==='SHORT'?side:null};
    }catch(_e){return {hasPosition:false,side:null};}
  }
  ...
  function powerGauge(reading){ const raw=Number(reading&&reading.current),value=clamp(Number.isFinite(raw)?raw:0,-100,100),previousRaw=reading&&reading.previous,prev=Number.isFinite(previousRaw)?clamp(previousRaw,-100,100):null; const cx=74,cy=60,radius=56,a=gaugeAngle(value); const tip=polar(cx,cy,52,a); const baseLen=13,halfBase=3.7,rad=a*Math.PI/180; const bx=cx+baseLen*Math.cos(rad+Math.PI), by=cy+baseLen*Math.sin(rad+Math.PI); const pxv=Math.cos(rad+Math.PI/2)*halfBase, pyv=Math.sin(rad+Math.PI/2)*halfBase; const points=`${tip.x.toFixed(1)},${tip.y.toFixed(1)} ${(bx+pxv).toFixed(1)},${(by+pyv).toFixed(1)} ${(bx-pxv).toFixed(1)},${(by-pyv).toFixed(1)}`; let ghost=''; if(prev!==null){ const ga=gaugeAngle(prev), grad=ga*Math.PI/180, gtip=polar(cx,cy,50,ga), gbx=cx+12*Math.cos(grad+Math.PI), gby=cy+12*Math.sin(grad+Math.PI), gpx=Math.cos(grad+Math.PI/2)*3.3, gpy=Math.sin(grad+Math.PI/2)*3.3; ghost=`<polygon class="ghostNeedle" data-gauge-previous="${prev}" points="${gtip.x.toFixed(1)},${gtip.y.toFixed(1)} ${(gbx+gpx).toFixed(1)},${(gby+gpy).toFixed(1)} ${(gbx-gpx).toFixed(1)},${(gby-gpy).toFixed(1)}"/>`; } let ticks=''; for(let i=-100;i<=100;i+=20){ const aa=gaugeAngle(i), p1=polar(cx,cy,45,aa), p2=polar(cx,cy,53,aa); ticks += `<line class="tick" x1="${p1.x.toFixed(1)}" y1="${p1.y.toFixed(1)}" x2="${p2.x.toFixed(1)}" y2="${p2.y.toFixed(1)}"/>`; } return `<div class="sssc-gauge" data-gauge-current="${value}" data-gauge-previous="${prev==null?'':prev}"><svg viewBox="0 0 148 68" aria-label="Direction-relative momentum ${signedText(value)}"><path class="gaugeInner" d="M 18 60 A 56 56 0 0 1 130 60 L 74 60 Z"/><path class="arc" d="M 18 60 A 56 56 0 0 1 130 60"/><path class="arcHi" d="M 18 60 A 56 56 0 0 1 130 60"/><g>${ticks}</g>${ghost}<polygon class="needle" points="${points}"/><rect class="hub" x="51" y="48" width="46" height="20" rx="0" ry="0"/><text class="svgval" x="74" y="58">${signedText(value)}</text></svg></div>`; }
  function chip(txt){ const s=String(txt||'-'); let cls=''; if(/Bull|Above/i.test(s))cls='bull'; if(/Bear|Below/i.test(s))cls='bear'; if(/Fresh|Wide Separation|Expansion/i.test(s))cls='blue'; if(/Failed|Tight|Contraction|Old/i.test(s))cls='warn'; return `<span class="sssc-ui-chip ${cls}">${s}</span>`; }
  ...
  function detail(tf){
    ...
    grid.innerHTML=`<div class="sssc-ui-detail-box"><b>Values</b>\nPrice: ${fmt(d.price,2)}\nVWAP: ${d.vwap==null?'Unavailable':fmt(d.vwap,2)}\n${emaLines}</div><div class="sssc-ui-detail-box"><b>SSSC</b>\nDirection: ${signed(d.direction)}\nDirectional strength: ${signed(d.directionalStrength)}\nExpansion / contraction: ${signed(d.expansionContraction)}\nDirectional acceleration: ${signed(d.directionalAcceleration)}\nStack: ${fmt(d.stackDir,0)}\nSlope dir: ${fmt(d.slopeDir,0)}\nSpread dir: ${fmt(d.sprDir,0)}\nSlope expansion power: ${fmt(d.slopePow,0)}\nSpread expansion power: ${fmt(d.sprPow,0)}\nSigned slope acceleration: ${fmt(d.signedSlopeAcceleration,0)}\nSigned spread acceleration: ${fmt(d.signedSpreadAcceleration,0)}</div><div class="sssc-ui-detail-box"><b>Spreads / Crosses</b>\n${spreadLines}\n\n${crossLines}</div><div class="sssc-ui-detail-box"><b>States</b>\n${eventLines}</div><div class="sssc-ui-detail-box"><b>Phase</b>\nState: ${d.state}\nPhase: ${d.phase}\nStrength: ${d.strengthState}\nExpansion / contraction: ${d.accelerationState||'-'}\nRows: ${d.rows}</div><div class="sssc-ui-detail-box"><b>Implication</b>\n${d.direction>35?'Bullish side favored.':d.direction<-35?'Bearish side favored.':'Mixed / wait for acceptance.'}\n${d.directionalStrength>25?'Bullish directional strength is meaningful.':d.directionalStrength<-25?'Bearish directional strength is meaningful.':'Directional strength is low.'}\n3M/1M warnings do not override 15M/1H/4H unless confirming failure/rejection/acceptance.</div>`;
  }
  ...
  function syncSsscToolbarButton(){
    const toolbarButton=document.querySelector('.topbar > #ssscDashBtn');
    if(toolbarButton){
      document.getElementById('ssscSettingsControlCard')?.remove();
      document.getElementById('ssscSettingsPanel')?.remove();
      document.getElementById('ssscSettingsTab')?.remove();
      if(!toolbarButton.__ssscBound){toolbarButton.__ssscBound=true;toolbarButton.addEventListener('click',()=>visible?hide():show());}
      toolbarButton.textContent=visible?'Hide SSSC':'SSSC';
      toolbarButton.title=visible?'Hide SSSC MA Diagnostic Dashboard':'Open SSSC MA Diagnostic Dashboard';
      return;
    }
  }
