...
async function serverTime(){
  try{
    const r = await fetch(cfg().time,{cache:"no-store"});
...
async function signedGet(url,p,key,sec,off){
  ...
  const r = await fetch(url + "?" + q + "&signature=" + sig,{
    method:"GET",
    cache:"no-store",
...
- [reason to avoid entry if invalidated]`;
  ...
  async function fetchJson(url,timeoutMs){ const controller=new AbortController(); const timer=setTimeout(()=>controller.abort(),timeoutMs||TIMEOUT_MS); try{ const r=await fetch(url,{cache:"no-store",headers:{"Cache-Control":"no-cache","Pragma":"no-cache"},signal:controller.signal}); const data=await r.json().catch(()=>null); if(!r.ok) throw new Error((data&&data.msg)?data.msg:"HTTP "+r.status); return data; }finally{ clearTimeout(timer); } }
...
(() => {
  ...
  const MA_STACK_STRIP = (() => {
    ...
    async function fetchTf(tf){
      ...
      try{ const r=await fetch(url,{cache:"no-store",signal:ctrl.signal}); if(!r.ok) throw new Error("HTTP "+r.status); const rows=await r.json(); return Array.isArray(rows)?rows:null; }
...
(() => {
  ...
  async function fetchTf(tf,count){ if(tf===iv() && Array.isArray(candles) && candles.length>=Math.min(80,count) && candles[0] && candles[0].close!=null){ return candles.slice(-count).map(c=>({time:c.time,open:+c.open,high:+c.high,low:+c.low,close:+c.close,volume:+c.volume,baseVolume:+c.volume,quoteVolume:null,fromChart:true})); } const url=restUrl()+'?symbol='+encodeURIComponent(sym())+'&interval='+encodeURIComponent(tf)+'&limit='+Math.min(1000,count)+'&endTime='+Date.now(); const r=await fetch(url,{cache:'no-store',headers:{'Cache-Control':'no-cache','Pragma':'no-cache'}}); if(!r.ok)throw new Error(tf+' HTTP '+r.status); const rows=await r.json(); if(!Array.isArray(rows))throw new Error(tf+' invalid data'); return rows.map(parseKline); }
  ...
  function connectWs(){ try{ if(ssscWs){ssscWs.close();ssscWs=null;} }catch(_e){} const lower=sym().toLowerCase(); const streams=TFS.map(x=>lower+'@kline_'+x[1]).join('/'); const url=wsUrl()+'?streams='+streams; try{ ssscWs=new WebSocket(url); }catch(e){ console.warn('SSSC WS create failed',e); return; } ssscWs.onmessage=e=>{ try{ const msg=JSON.parse(e.data); const d=msg.data||msg; if(!d.k)return; lastWs=Date.now(); const tf=d.k.i; upsertBuffer(tf,parseWsK(d.k),420); }catch(err){ console.warn('SSSC WS parse failed',err); } }; ssscWs.onclose=()=>{ if(visible) setTimeout(()=>visible&&connectWs(),3000); }; ssscWs.onerror=()=>{}; }
