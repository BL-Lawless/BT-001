(() => {
  ...
  function directionEvidence(snapshot,profile){
    ...
    const long=evaluate("LONG"),short=evaluate("SHORT"),winner=long.score>=short.score?long:short,runner=winner===long?short:long,permission=winner.score>=58&&winner.score-runner.score>=5;
    const earlyOpposite=candleFlow(closedRows(snapshot,profile.early),winner.direction==="LONG"?"SHORT":"LONG",6),lowerTimeframeWarning={active:earlyOpposite.effective,direction:winner.direction==="LONG"?"SHORT":"LONG",evidence:earlyOpposite.evidence};
    return {direction:permission?winner.direction:null,score:permission?winner.score:Math.max(long.score,short.score),longScore:long.score,shortScore:short.score,permission,breakdown:{...winner.breakdown,lowerTimeframeWarning},primaryFlow:winner.flow,lowerTimeframeWarning,reason:permission?`${winner.direction} permission from primary/structural agreement`:`Directional evidence is mixed (${long.score.toFixed(0)} LONG / ${short.score.toFixed(0)} SHORT)`};
  }
  ...
  function extractFacts(snapshot,horizonId,state){
    ...
    return {fresh:["LIVE","SUFFICIENT"].includes(fresh),directionalPermission:direction,setup:setup?{...setup,valid:setup.quality>=60}:null,setupCandidates:candidates.map(item=>({family:item.family,tf:item.tf,quality:item.quality,distanceAtr:item.distanceAtr,interacted:item.interacted,nonEma:item.nonEma})),setupComponents,trigger,opposition,volatility:primaryVolatility,geometry,current:{originDistanceAtr,chased:originDistanceAtr>profile.chaseAtr,adverseEvidence:opposition.effective?70:15},profile};
  }
  function scoreFacts(facts,{horizonId="quick",symbol="",version=0}={}){
    const profile=facts.profile||PROFILES[horizonId]||PROFILES.quick,permission=facts.directionalPermission||{},setup=facts.setup,trigger=facts.trigger||{},flow=trigger.flow||{},participation=trigger.participation||{},geometry=facts.geometry||{},volatility=facts.volatility||{},current=facts.current||{};
    ...
    const gates={
      freshData:facts.fresh===true,directionalPermission:permission.permission===true&&!!permission.direction,validSetup:!!setup&&setup.valid!==false,setupInteraction:!!setup&&setup.interacted===true,closedReaction:!!setup&&setup.reactionConfirmed===true,
    ...
    if(setup&&setup.invalidated){stateName="SETUP FAILED";stateReason="Closed structural invalidation breached the setup";}
    else if(!permission.permission){stateName="NO SETUP";stateReason=permission.reason||"Directional permission is unavailable";}
    ...
    const presentation=PRESENTATION[stateName],direction=permission.direction||"NO BIAS",triggerIdentity=stateName==="TRIGGER ACTIVE"?[symbol,horizonId,direction,setup.identity,profile.trigger,trigger.shiftTime||version].join("|"):null;
    const reasons=unique([stateReason,...(trigger.evidence||[]),...(facts.opposition&&facts.opposition.evidence||[])]),exclusions=failed.map(name=>`Hard gate failed: ${name}`);
    ...
    const comparisonDiagnostics={directionalPermissionScore:num(permission.score),directionalPermissionBreakdown:permission.breakdown||{},setupScore,setupBreakdown,triggerScore,triggerBreakdown,currentEntryScore:currentScore,currentEntryBreakdown:currentBreakdown,hardGates:{passed:Object.entries(gates).filter(([,value])=>value).map(([name])=>name),failed},effectiveOppositionEvidence:facts.opposition&&facts.opposition.evidence||[],volatilityRegime:volatility.regime||"Unavailable",realizedRangePercentile:num(volatility.realizedRangePercentile),participationState:participation.state||"UNAVAILABLE",participation,flowEffectiveness:flow,chaseDistanceAtr:num(current.originDistanceAtr),remainingRewardRisk:num(geometry.netRr),finalStateReason:stateReason,engineVersion:VERSION,publicationGeneration:null};
  ...
  function createSignalEngineB(){
    ...
    const engine={
      ...
      evaluate(context={}){const snapshot=context.snapshot;if(!snapshot)throw new Error("Signal B market snapshot is unavailable");const horizonId=PROFILES[context.horizonId]?context.horizonId:"quick",fingerprint=[snapshot.signature||snapshot.version,horizonId].join("|");if(state.evaluationCache.has(fingerprint)){cacheHits+=1;return state.evaluationCache.get(fingerprint);}calculations+=1;const facts=extractFacts(snapshot,horizonId,state),result=scoreFacts(facts,{horizonId,symbol:snapshot.symbol,version:snapshot.version});result.comparisonDiagnostics.publicationGeneration=Number(context.publicationGeneration)||0;lastDiagnostics=result.comparisonDiagnostics;state.evaluationCache.set(fingerprint,result);state.fingerprints.set(horizonId,fingerprint);while(state.evaluationCache.size>24)state.evaluationCache.delete(state.evaluationCache.keys().next().value);return result;},
      evaluateFacts(facts,metadata={}){return scoreFacts(facts,metadata);},extractFacts(snapshot,horizonId="quick"){return extractFacts(snapshot,horizonId,state);},
