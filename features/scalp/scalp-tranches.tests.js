...
async function run(){
  ...
  assert(logs.filter(item=>item.table==="scalp_activity_log"&&item.row.action==="TRANCHE_ADDED").length===6);assert(logs.some(item=>item.row.action==="TRANCHE_CLOSED"&&item.row.position_state.trancheId===firstLong.trancheId));
  ...
  assert.equal(lockingTranche.profitLockTriggered,true);assert.equal(lockingTranche.profitLockPrice,lockLevel);assert.equal(lockingTranche.profitLockClosedQty,.04);assert.equal(lockingTranche.remainingQty,.06);assert(lockingTranche.feeAwareBreakevenPrice>lockingTranche.entryPrice);const remainingNet=(lockingTranche.feeAwareBreakevenPrice-lockingTranche.entryPrice)*lockingTranche.remainingQty-(lockingTranche.entryCommission*lockingTranche.remainingQty/lockingTranche.filledQty)-lockingTranche.feeAwareBreakevenPrice*lockingTranche.remainingQty*profitEngine.rates.taker;assert(remainingNet>=-1e-12);assert(profitGateway.algo.some(row=>row.clientAlgoId===lockingTranche.pslClientId&&row.quantity==="0.060"&&Number(row.triggerPrice)===lockingTranche.feeAwareBreakevenPrice));assert(profitGateway.normal.some(row=>row.clientOrderId===lockingTranche.partialTpClientId&&row.origQty==="0.060"));assert.equal(untouchedSibling.remainingQty,.1);assert.equal(untouchedSibling.pslOrderId,siblingPsl);assert.equal(untouchedSibling.partialTpOrderId,siblingTp);assert.equal(untouchedSibling.profitLockEnabled,false);assert(logs.some(item=>item.table==="scalp_activity_log"&&item.row.action==="PROFIT_LOCK_APPLIED"&&item.row.position_state.trancheId===lockingTranche.trancheId));
