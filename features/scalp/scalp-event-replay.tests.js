...
async function run(){
  ...
  const bearishClose=crossHub.closeForSeparation("1m","SHORT",0.12,tools);crossHub.emitForming("1m",bearishClose);let row=crossEngine.snapshot().detections.find(item=>item.source==="1m");assert.equal(row.direction,"SHORT");assert.equal(row.eventType,"CROSS");assert.equal(row.phase,"LIVE");assert.equal(row.qualified,true);assert(Number.isInteger(row.rankValue));assert(row.rankDiagnostics&&row.rankDiagnostics.pressureAvailable);assert(row.raw.atr>0&&row.raw.priorAtr>0&&Number.isFinite(row.raw.atrChange));assert(!Object.prototype.hasOwnProperty.call(row.raw,"range"));cases.bearishIntrabarCrossImmediate=true;

  const bullishHub=new CanonicalReplayHub();for(const tf of ["1m","3m","5m","15m"])bullishHub.seed(tf,trend("SHORT"));const bullishEngine=await initializedEngine(build,context,bullishHub,{now:()=>++clock});const bullishClose=bullishHub.closeForSeparation("1m","LONG",0.12,tools);bullishHub.emitForming("1m",bullishClose);row=bullishEngine.snapshot().detections.find(item=>item.source==="1m");assert.equal(row.direction,"LONG");assert.equal(row.eventType,"CROSS");assert.equal(row.qualified,true);assert(!build.config.signal.crossPersistenceMs);cases.bullishCrossImmediateWithoutCloseOrTimer=true;
