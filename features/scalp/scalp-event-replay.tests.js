...
async function run(){
  ...
  const sourceHub=new CanonicalReplayHub();for(const tf of ["1m","3m","5m"])sourceHub.seed(tf,trend("LONG"));const sourceEngine=await initializedEngine(build,context,sourceHub,{now:()=>++clock});sourceEngine.config={...sourceEngine.config,source:"1m"};sourceHub.emitForming("3m",sourceHub.closeForSeparation("3m","SHORT",0.05,tools));row=sourceEngine.snapshot().detections.find(item=>item.source==="3m");assert.equal(row.eventType,"CROSS");assert.equal(row.direction,"SHORT");assert.equal(row.eligibility,"SOURCE FILTER");assert(sourceHub.requirements.some(item=>item.requirements.every(req=>["1m","3m","5m","15m"].includes(req.tf))));cases.nonSelectedAndOffChartDetectionRemainPopulated=true;
  ...
  const exitHub=new CanonicalReplayHub();for(const tf of ["1m","3m","5m"])exitHub.seed(tf,trend("LONG"));const exitEngine=await initializedEngine(build,context,exitHub,{now:()=>++clock});exitEngine.state="ACTIVE";exitEngine.config={...exitEngine.config,minimumRank:100};exitEngine.session={symbol:"BTCUSDT",direction:"LONG",source:"1m",eventId:"owned",liveQty:.1};let exitReason=null;exitEngine.requestExit=async reason=>{exitReason=reason;};exitHub.emitForming("1m",exitHub.closeForSeparation("1m","SHORT",0.05,tools));assert.equal(exitReason,"OPPOSITE_CROSS");cases.oppositeLiveCrossIgnoresRankGateAndExitsOwnedPosition=true;
  ...
  for(const engine of [crossEngine,bullishEngine,fifteenEngine,allTfEngine,projectedEngine,frozenBounce.engine,priceTouchEngine,flatEngine,invalidEngine,blockedEngine,blockedBounce.engine,sourceEngine,retainedEngine,reconnectEngine,exitEngine])engine.destroy();
