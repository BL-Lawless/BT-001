...
assert(rapid.includes('id="rapidFireTakeProfitSet" type="button">Set</button>')&&rapid.includes("const typedValue=takeProfitCommitValue(protectionPriceSubmissionValue(tpInput),takeProfitEditValue)")&&rapid.includes('commitProtection("tp",tpInput,typedValue)'),'TP Set must commit the captured exact edit rather than rounded display text or a blur-reverted field value');
