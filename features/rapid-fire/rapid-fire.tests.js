...
assert(rapid.includes('actionLabels={add:"Add",double:"Double",close:"Close",reverse:"Reverse"'),"Close and Reverse must return to their action titles when idle");
...
assert(/\.rapid-fire-lot\{[^}]*width:100%/s.test(css)&&css.includes("grid-template-columns:74px 86px 50px 62px 44px"),"Add's lot input must occupy a compact fixed-width grid column");
assert(/\.rapid-fire-status\{[^}]*width:100%;[^}]*height:34px/s.test(css),"status must be a full-width single-line bottom row");
assert(css.includes(".rapid-fire-slider-ticks i")&&rapid.includes("sliderTicks(REVERSE_PERCENT_STEPS,25,300)"),"both sliders must render discrete tick markers");
