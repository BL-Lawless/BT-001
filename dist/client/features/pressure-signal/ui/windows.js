(() => {
  ...
  build.createWindowSystem = function createWindowSystem(config,format){
    ...
    function signalMetaMatches(left,right){
      ...
      const leftMode=left.directionMode||left.mode,rightMode=right.directionMode||right.mode;
      return Number(left.generation)===Number(right.generation) && left.signalIdentity===right.signalIdentity
        && leftMode===rightMode && (left.evaluatedDirection||left.direction)===(right.evaluatedDirection||right.direction) && (left.authoritativePhase||null)===(right.authoritativePhase||null) && left.direction===right.direction && (left.confidence==null ? null : Number(left.confidence))===(right.confidence==null ? null : Number(right.confidence))
        && left.visibleState===right.visibleState && (left.setupIdentity || null)===(right.setupIdentity || null)
        && left.horizonId===right.horizonId && left.engineId===right.engineId && (left.signalId||left.engineId)===(right.signalId||right.engineId) && left.engineVersion===right.engineVersion
    ...
    function buttonMatchesDisplayedSignal(displayed){
      ...
      const checks={
        directionMode:button.dataset.signalDirectionMode===displayed.mode,
        evaluatedDirection:button.dataset.signalEvaluatedDirection===(displayed.evaluatedDirection||displayed.direction),
        authoritativePhase:(button.dataset.signalAuthoritativePhase||null)===(displayed.authoritativePhase||null),
        ...
        engine:button.dataset.signalEngineId===displayed.engineId && button.dataset.signalId===(displayed.signalId||displayed.engineId) && button.dataset.signalEngineVersion===displayed.engineVersion,
