(() => {
  ...
  function simulationConfig(input={}){
    ...
    return {
      ...
      moveSlToBeEnabled:input.moveSlToBeEnabled===true||input.profitLockEnabled===true,beThresholdPct:Math.max(1,Math.min(100,n(input.beThresholdPct??input.lockThresholdPct)??C.defaults.beThresholdPct)),closePortionEnabled:input.closePortionEnabled===true||input.profitLockEnabled===true,closeThresholdPct:Math.max(1,Math.min(100,n(input.closeThresholdPct??input.lockThresholdPct)??C.defaults.closeThresholdPct)),closePortionPct:Math.max(1,Math.min(99,n(input.closePortionPct??input.lockPortionPct)??C.defaults.closePortionPct)),
  ...
  function openTranche(book,event,candle,config,sequence){
    const qty=config.lot,entryPrice=candle.close,entryCommission=entryPrice*qty*config.rates.taker,base={trancheId:`SIM-${event.direction[0]}-${sequence}`,direction:event.direction,source:event.sourceTimeframe,eventId:event.eventId,eventType:event.eventType,triggerRank:event.rank,requestedQty:qty,filledQty:qty,remainingQty:qty,entryPrice,entryCommission,status:"ACTIVE",createdAt:event.signalCloseTimeMs,moveSlToBeEnabled:config.moveSlToBeEnabled,beThresholdPct:config.beThresholdPct,closePortionEnabled:config.closePortionEnabled,closeThresholdPct:config.closeThresholdPct,closePortionPct:config.closePortionPct,beMoveTriggered:false,closePortionTriggered:false,tradeManagementPending:false,rankBoostEnabled:config.rankBoostEnabled,rankBoostThreshold:config.rankBoostThreshold,rankBoostPoints:config.rankBoostPoints,realizedPnlUsd:0,mddUsd:0};
  ...
  function applyProfitLock(tranche,candle,config){
    const triggerPrice=tranche.direction==="LONG"?candle.high:candle.low,lock=decisions.profitLockDecision({tranche,price:triggerPrice,filters:config.filters}),moveBe=decisions.beReached({tranche,price:triggerPrice,tickSize:config.filters.tickSize});
    if(!lock.reached&&!moveBe)return false;
    const closeQty=lock.reached?lock.quantity:0,before=n(tranche.remainingQty)||0,remaining=calc.normalizeLot(Math.max(0,before-closeQty),config.filters);if(closeQty>0){tranche.realizedPnlUsd=(n(tranche.realizedPnlUsd)||0)+pnlAt(tranche,closeQty,lock.level,config.rates.taker);tranche.closePortionTriggered=true;tranche.profitLockClosedQty=closeQty;tranche.remainingQty=remaining;}if(moveBe){const allocatedEntryCommission=(n(tranche.entryCommission)||0)*(remaining/Math.max(remaining,n(tranche.filledQty)||remaining)),breakeven=calc.feeAwareBreakeven({direction:tranche.direction,entryPrice:tranche.entryPrice,qty:remaining,entryCommission:allocatedEntryCommission,exitRate:config.rates.taker,tickSize:config.filters.tickSize});tranche.beMoveTriggered=true;tranche.feeAwareBreakevenPrice=breakeven;tranche.pslPrice=breakeven;}
    tranche.profitLockTriggered=!!(tranche.closePortionTriggered||tranche.beMoveTriggered);return true;
  }
